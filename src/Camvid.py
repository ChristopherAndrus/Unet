...
class CamVid_Dataset():
  def __init__(self, img_pth, mask_pth, transform):
    ...
    all_masks = os.listdir(self.img_pth)
    #all_masks = [img_name[:-4] + '_L' + img_name[-4:] for img_name in all_imgs]
    random.seed(43)
    self.a = random.sample( natsort.natsorted(all_imgs), len(all_imgs) )
    self.total_imgs = self.a
    random.seed(43)
    self.b = random.sample( natsort.natsorted(all_masks), len(all_masks) )
    self.total_masks = self.b
