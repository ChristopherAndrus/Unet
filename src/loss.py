...
ALPHA = 1e2 #just make number not too small
GAMMA = 2 #cause close
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, alpha=ALPHA, gamma=GAMMA, smooth=1):
        ...
        focal_loss = alpha * (1-BCE_EXP)**gamma * BCE #*BCE
        print(focal_loss)
...
class OHEMLoss(nn.Module):
    ...
    def forward(self, inputs, targets, keep_num= 5):
        loss = torch.nn.CrossEntropyLoss(reduce = False)(inputs, targets)
