...
if __name__ == "__main__":
    ...
    #criterion = SrcLoss.SSLoss()
    criterion = torch.nn.CrossEntropyLoss()

    imgs, masks, pred = Test_eval(model, testloader, criterion, load_model_pth, device)
    print(imgs.shape, masks.shape, pred.shape)

    Visualize(imgs, 'Original Image', 5, 1, change_dim=True)
    Visualize(masks, 'Original Mask', 5, 1, change_dim=True)
    Visualize(pred, 'Predicted mask', 5, 1, change_dim=False)